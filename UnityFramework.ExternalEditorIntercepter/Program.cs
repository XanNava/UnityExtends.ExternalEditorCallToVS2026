// taskkill /IM UnityFramework.ExternalEditorIntercepter.exe /F
...
namespace VsInterceptor {
	internal static class Program {
		...
		// ─────────────────────────────────────────────────────────────────────
		// NATIVE: CONSOLE + ROT + WINDOW FOCUS
		// ─────────────────────────────────────────────────────────────────────
		...
		private static void TryActivateMainWindow(object dteObject, Type dteType) {
			try {
				...
				// First, let EnvDTE try to activate.
				LogText("[OpenFileViaDte] Activating MainWindow via EnvDTE.Window.Activate()...");
				mainWindowType.InvokeMember("Activate",
					System.Reflection.BindingFlags.InvokeMethod, null, mainWindowObject, null);

				// Then, try to bring the underlying HWND to the foreground.
				try {
					object? hwndPropertyValue = mainWindowType.GetProperty("HWnd")?.GetValue(mainWindowObject);
					if (hwndPropertyValue is int hwndInt && hwndInt != 0) {
						IntPtr windowHandle = new IntPtr(hwndInt);
						LogText("[OpenFileViaDte] MainWindow.HWnd=" + hwndInt +
								" -> calling ShowWindowAsync(SW_RESTORE) + SetForegroundWindow");

						ShowWindowAsync(windowHandle, SW_RESTORE);
						SetForegroundWindow(windowHandle);
					} else {
						LogText("[OpenFileViaDte] MainWindow.HWnd is null or 0; skipping user32 foreground calls.");
					}
				}
				catch (Exception windowHandleException) {
					LogText("[OpenFileViaDte] ERROR while using HWnd to bring window to front: " + windowHandleException);
				}
			}
			catch (Exception exception) {
